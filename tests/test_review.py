...
def test_deep_dive_is_catalog_honest():
    ...
    assert "Client twin." in body
    assert "Close bench." in body
    assert "Operating company." in body
    assert "Brand." in body
    assert "Client universe." in body
    assert "Honest zeros" in body
    assert "Refuse is visible" in body
    assert "Sit-down client day" in body
    assert "Industry drawer." in body
    assert "Sit / maps / attach / refuse" in body
    assert "Room 1 is books" in body
    assert "Room 2 is refuse" in body
    assert "Honest zeros" in body
    assert "Refuse is visible" in body
    assert "Every refuse clicks" in body
    assert "Catalog is the message" in body
    assert "Complete industry drawer" in body
...
def test_review_json_cannot_claim_live():
    ...
    assert len(card["expert_review"]["upgrades"]) == 71
