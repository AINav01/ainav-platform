...
def test_release_is_285_first_class_close_bench():
    ...
    assert len(cat["expert_review"]["upgrades"]) == 55
    ...
    assert "2.85.0" in html
    assert 'id="close-console"' in html
    assert 'id="path-planes"' in html
    dash = public_dashboard()
    assert dash["release"] == "2.85.0"
    status = public_status()
    assert status["release"] == "2.85.0"
...
def test_instrument_285_fail_closed():
    ...
    hole["entity"]["release"] = "2.84.0"
    with pytest.raises(IntegrityError):
        catmod._validate_instrument_285(hole, hole["plane_interface"])
