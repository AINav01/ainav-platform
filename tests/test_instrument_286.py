...
def test_release_is_286_first_class_operating_company():
    ...
    assert len(cat["expert_review"]["upgrades"]) == 56
    ...
    assert "2.86.0" in html
    assert 'id="firm-console"' in html
    assert 'id="firm-rails"' in html
    dash = public_dashboard()
    assert dash["release"] == "2.86.0"
    status = public_status()
    assert status["release"] == "2.86.0"
...
def test_instrument_286_fail_closed():
    ...
    hole["entity"]["release"] = "2.85.0"
    with pytest.raises(IntegrityError):
        catmod._validate_instrument_286(hole, hole["plane_interface"])
