...
Release 3.28.0. Not a SKU. Not LIVE_PIN_OK. Not a certificate.
...
Every new client AI is another unauthorized-write surface unless one human plane sits over all of them.
...
One dashboard included with L1. Hierarchical views are the same plane. Not a second dashboard SKU.

- **Seat A** — Treasury admits
- **Seat B** — Controller admits
- **One hash** — Consume once
- **Then the write** — Fail-closed SoR
...
The sale is the ninety-minute proof. Two existing treasury seats. Client AI or a customer AI may draft. Walk out: sealed DecisionRecord; Merkle / audit export. Lab oids are not two named treasury humans. Not signed L1. Not LIVE_PIN_OK.
...
- **Client executive** — The named controller. Must-have: every new client AI is another unauthorized-write surface unless one human plane sits over all of them.. Can: Sit the ninety-minute proof-day Floor: write rail, attention, seats, keep, and included versus upsell. Same dashboard included with L1. Cannot: Treat the dashboard as a SKU. Treat standard provision as a SKU. Treat advanced provision as a SKU. Attach U-DUAL free. Invent a buyer.
...
| Signal | Value | Note |
| --- | --- | --- |
| Must-have | one plane over every new client AI | Every new client AI is another unauthorized-write surface unless one human plane sits over all of them. Not a mandate. Not a certificate. |
...
- **Seat** — Bind an action_hash. Standing: false. Own Entra object id. Own click. 1 mailbox recorded / 0 oid. Mailbox is not the click.
...
- **Other uses** — same Job C. Lead stays bc.general_journal.post; Prove — L1 GL desks; Deepen — paid U-DUAL commercial writes; Keep — P-ADM second record
- **Failsafe and oversee** — not the client's AI. Admit · off switch · reset · rollback; Owner and board oversee; They are not seats; One title cannot click both admits
- **Records and maps** — claimed=false. First record is the SoR write; Second record is the sealed DecisionRecord; Immutable: sealed, consume-once, hash-chained; AI governance maps. Counsel stays open (G12)
...
- **Included with L1** — inside $28–40k · 2–4 weeks. SKU: false. Upsell: false. Week-one prove: L1 + industry.treasury + lib.l1.wedge on the BC twin; Every L1 pack and library with included_in_sku=true; Entra identity, Teams notify, twin, proof day / Acceptance Kit; Client executive dashboard; Same dashboard included with L1 — encyclopedia is a drawer on Entire; Proof-day Client stays write rail, attention, seats, keep, and offer; Standard provision seating
...
| Tile | Value | Note |
| --- | --- | --- |
| Must-have | one plane over every new client AI | Every new client AI is another unauthorized-write surface unless one human plane sits over all of them. |
