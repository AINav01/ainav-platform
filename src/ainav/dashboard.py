...
def public_dashboard() -> dict[str, Any]:
    ...
    return {
        ...
        "lab_vs_commercial": dict(body.get("lab_vs_commercial") or {}),
        "board_packet": dict(body.get("board_packet") or {}),
...
def dashboard_markdown() -> str:
    ...
    lines += [
        ...
        "",
        f"{((body.get('floor') or {}).get('accountable') or {}).get('lede') or ''}",
        "",
    ]
    for item in ((body.get("floor") or {}).get("accountable") or {}).get("items") or []:
        lines.append(f"- **{item.get('name')}** — {item.get('note') or ''}")
    lines += [
        "",
    ...
    lines += [
        ...
        "",
        f"{((body.get('floor') or {}).get('memory') or {}).get('lede') or ''}",
        "",
    ]
    for item in ((body.get("floor") or {}).get("memory") or {}).get("items") or []:
        lines.append(f"- **{item.get('name')}** — {item.get('note') or ''}")
    lines += [
        "",
        "### Complete the stack",
        "",
        f"{((body.get('floor') or {}).get('integrate') or {}).get('lede') or ''}",
        "",
    ]
    for item in ((body.get("floor") or {}).get("integrate") or {}).get("items") or []:
        label = item.get("url_label") or item.get("name") or ""
        url = item.get("url") or ""
        lines.append(f"- **{label}** — {item.get('note') or ''} {url}")
    lines += [
        "",
